 ...
 * job (see `UserService.queueInvitationEmail()` — `subject` is computed by
 * the caller and sent as-is, this registry only renders the body). Plain
 * template functions rather than a templating engine/file-based views —
 * there is exactly one template today (`user-invitation`); a real engine
 * (Handlebars/MJML) is worth adding once there are enough of these to justify
 * one, not before.
 * ─────────────────────────────────────────────────────────────────────────────
 */
