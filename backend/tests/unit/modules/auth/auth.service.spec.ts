 ...
 * for why @config/database is stubbed — AuthRepository is injected as a full
 * mock via the constructor below, so the real singleton is never touched,
 * but importing anything under `@modules/*` still transitively imports it.
 */
...
import type { ChangePasswordDto, LoginDto, RequestMeta } from '@modules/auth/dto/auth.req.dto';
...
function createService(repository: MockedAuthRepository, req: Request = createFakeRequest()): AuthService {
  return new AuthService(req, repository as unknown as AuthRepository);
}
